...
let rotateAnimationPlayFlag = ref(true);
let animationPosition = ref<any>({});
// 控制开始结束更快更慢
...
function cesiumShowTrace(gpxml: string) {
  let gpxText = gpxml.replace(/GPX\/1\/0\"/g, 'GPX/1/1"');
  let parser = new DOMParser();
  let xmlObject = parser.parseFromString(gpxText, 'text/xml');
...
async function followTrace(dataSource: any) {
  ...
  let trackPoints3dTemp = [];
  let trackPoints3d = <any>[];
  viewer.dataSources.removeAll();

  // 查找轨迹点
  var trackPointsXml = dataSource.getElementsByTagName('trkpt');
  for (var i = 0; i < trackPointsXml.length; i++) {
    var trackPointXml = trackPointsXml[i];

    // 获取轨迹点的经纬度
    var latitude = parseFloat(trackPointXml.getAttribute('lat'));
    var longitude = parseFloat(trackPointXml.getAttribute('lon'));
    var time: any = dayjs().add(i, 'second').format('YYYY-MM-DDTHH:mm:ssZ');

    // 创建Cesium.Cartographic对象
    var cartographic = { longitude, latitude, time };
  ...
  viewer.dataSources.add(Cesium.CzmlDataSource.load(czml)).then((ds: any) => {
    ...
    viewer.clock.onTick.addEventListener(function (clock: any) {
      if (viewer.trackedEntity) {
        // 平面坐标系
        let cartesian = viewer.trackedEntity.position.getValue(
          clock.currentTime,
        );
        // 平面转弧度坐标系
